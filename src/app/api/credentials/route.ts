    ...
    fetch('/api/credentials')
      ...
      .then((data: { credentials: Credential[] }) => {
        const active = (data.credentials ?? []).filter(
          c => c.providerCode === 'google' && c.status === 'active'
        )
        ...
        <div className="space-y-1.5">
          ...
          {loadingCreds ? (
            ...
            <select
              ...
              className="w-full rounded-lg border border-gray-600 bg-gray-800 text-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50"
            >
              <option value="" style={{ background: '#1f2937', color: '#fff' }}>— เลือก API Key —</option>
              {credentials.map(c => (
                <option key={c.id} value={c.id} style={{ background: '#1f2937', color: '#fff' }}>{c.displayName}</option>
              ))}
            </select>
          )}
        </div>
