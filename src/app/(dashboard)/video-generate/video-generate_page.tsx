...
const PROVIDER_NOTES: Record<string, { type: 'warning' | 'info'; message: string }> = {
  google: {
    type:    'warning',
    message: '⚠️ Veo 3.1 ต้องใช้ Vertex AI (Google Cloud) — ยังไม่รองรับผ่าน AI Studio Key ปกติ',
  },
  'google-vertex': {
    type:    'info',
    message: '✅ Veo 3.1 via Vertex AI — พร้อมใช้งาน (ใช้ Service Account JSON)',
  },
  kling: {
    type:    'info',
    message: '📹 Kling รองรับ 5 หรือ 10 วินาทีเท่านั้น — เลือก "8 วินาที" จะถูกปรับเป็น 10 วินาทีอัตโนมัติ',
  },
  xai: {
    type:    'info',
    message: '🚀 xAI Grok Imagine Video — สร้างวิดีโอ text-to-video คุณภาพสูง สูงสุด 15 วินาที',
  },
}
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-1">
        <div>
          ...
          <p className="text-sm text-[#9C9690] mt-1">Kling AI · xAI Grok · Google Veo 3.1 (Vertex AI)</p>
        </div>
      ...
      {credentials.length === 0 && (
        <div className="mt-4 p-4 rounded-2xl border border-[#2C2A35] text-sm text-[#9C9690]">
          ...
          <strong className="text-bone">Kling AI</strong>,{' '}
          <strong className="text-bone">xAI Grok</strong> หรือ{' '}
          <strong className="text-bone">Google Vertex AI (Veo)</strong>{' '}
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5 max-w-xl">
        ...
        <div>
          <label className="block text-xs text-[#9C9690] mb-1.5">
            Prompt — บอกว่าต้องการวิดีโออะไร (แนะนำเป็น English)
          </label>
          <textarea
            required
            rows={4}
            ...
            disabled={isRunning}
            placeholder="เช่น: a butterfly flying over a flower field in the golden morning light, cinematic close-up"
        ...
        <div>
          ...
          <input
            ...
            disabled={isRunning}
            placeholder="เช่น: text, logo, blurry, low quality, watermark"
        ...
        {providerNote && (
          <div className={`p-3 rounded-xl border text-xs ${
            providerNote.type === 'warning'
              ? 'border-amber-500/30 bg-amber-500/10 text-amber-400'
              : 'border-[#2C2A35] bg-[#2C2A35]/30 text-[#9C9690]'
          }`}>
        ...
        {isRunning && (
          <p className="text-[10px] text-[#9C9690] text-center">
            กำลังสร้างวิดีโออยู่ — ใช้เวลา 2-8 นาที ระบบจะแจ้งเมื่อเสร็จอัตโนมัติ
          </p>
        )}
