...
const models = [
  ...
  // Google — image
  { providerCode: 'google', modelCode: 'gemini-2.5-flash-image',        displayName: 'Nano Banana — สร้างภาพ',          capability: 'image' },
  // Google — video
  ...
  { providerCode: 'xai', modelCode: 'grok-imagine-video-1.5', displayName: 'Grok Imagine Video 1.5',      capability: 'video' },
  // Anthropic — text
...
async function main() {
  ...
  for (const m of models) {
    await prisma.aiModel.upsert({
      ...
      create: { providerCode: m.providerCode, modelCode: m.modelCode, displayName: m.displayName, capability: m.capability, isActive: true }
    });
